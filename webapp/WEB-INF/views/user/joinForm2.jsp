...
$("#joinForm").on("blur", function(){
	var userIdLengthTrigger = false;
	var isPassUserId = false;
	var existUserIdResponse = false;

	var joinBtnActive = function(isActive) {
		$("#joinBtn").attr("class",
				"d-btn btn-primary" + (!isActive ? " disabled" : ""));
	};

	function checkUserIdLength() {
		if ($('#userId').val().length === 11 && userIdLengthTrigger) {
			existUserId(existUserIdResponse);
			return false;
		} else {
			userIdLengthTrigger = false;
			return true;
		}
	}
	...
	function existUserId(response) {
		...
		if (response.success) {
			...
			$userId.attr('class', 'd-input input-danger')
	...
	function validateUserId() {
		...
		if (!checkIdValue($userId.val())) {
			$("#hLayerid").removeClass('validation-pass').html(
					'아이디는 영문소문자, 숫자만 사용 가능합니다.');
			$userId.attr('class', 'd-input input-danger');
			isPassUserId = false;
			$("input[name='checkId']").val('0');
			joinBtnActive(false);
	...
	function ajaxExistUserId() {
		...
		$.ajax({
			...
			data : {userId: $userId.val(), joinToken: joinToken},
